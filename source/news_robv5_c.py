...
for page in range(1, pagesToGet + 1):
    ...
    for j in links:
        #print(j.get_text())
        headlines=j.get_text()
        mystring = headlines.replace('\n', ';').replace('\r', ';')
        ...
        pos=4
        vecthead=[]
        while headlines < (len(mystring)):
            mystringv=mystring[paso:paso+1]
            #print('headline',mystringv)
            vecthead.append(mystringv)
            #verif=str(mystringv)
            #print('boolean  ',bool(verif.strip()))
            #print('paso ',pos+paso+1)
            #headlinesv=mystring[pos+paso+1]
            #print('headlinev2 ',headlinesv)
            paso=paso+pos
            headlines=headlines+1
            #print('contador  ', headlines)
